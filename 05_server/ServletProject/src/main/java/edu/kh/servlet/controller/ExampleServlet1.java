...
public class ExampleServlet1 extends HttpServlet{
	...
	// 2) 요청을 처리할 method 타입에 따라서 doGet() 또는 doPost() 메서드를 오버라이딩
	// 	  -> 자동 완성 시 내부 코드 모두 삭제
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// String req.getParameter("input의 name 속성 값);
		...
		// ** HttpServletResponse 이용해서 HTML 문서를 만들어서 응답! **
		
		// 1) 응닶하는 문서의 형식과 문자 인코딩 지정하기
		resp.setContentType("text/html; charset=UTF-8");
		// -> 글로 작성된 HTML 문서, 문자 인코딩 형식은 UTF-8로 지정
		
		// 2) 서버 -> 클라이언트로 HTML을 전할하기 위한 출력 스트림 얻어오기
		PrintWriter out = resp.getWriter();
		
		// 3) 출력 스트림을 이용해서 HTML 코드를 출력
		out.println("<!DOCTYCE html>");
		out.println("<html>");
