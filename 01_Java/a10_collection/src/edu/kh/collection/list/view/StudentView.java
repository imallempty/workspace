...
public class StudentView {
	...
	private void addStudent() {
		...
		boolean result = service.addStudnet(grade, ban, number, name, gender, score);
	...
	private void selectAllStudent() {
		...
		// Service로 부터 List<Student>   객체의 참조 주소 반환 받기
	...
	private void selectOne() {
		...
		System.out.print("조회할 할 학생의 index 입력 : ");
	...
	private void deleteStudent() {
		...
		System.out.println("/n----- 학생 1명 삭제(index) -----\n");
		
		// 인덱스 번호를 입력 받아서 Service 메서드를 호출
		// List에서 해당하는 인덱스 학생이 있으면 
		// List에서 삭제 후 삭제된 학생을 반환
		// --> "000 학생의 정보가 삭제되었습니다" 출력
		
		// List에서 해당하는 인덱스가 없으면
		// null 반환
	...
	private void selectGender() {
		...
		// M 또는 F를 입력한 경우
		// Service의 성별 조회 메서드 호출(입력 받은 성별 전달)
...
//		if(searchList.isEmpty())
	...
	private void selectGrade() {
		...
		// 학년은 1 ~ 6학년까지만 존재
		if(grade < 1 || grade > 6) {
		System.out.println("* 학년은 1 ~ 6학년까지만 존재합니다. *");
		return;
		}
