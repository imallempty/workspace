...
public class SwitchEx {
	...
	public void ex2() {
		...
		// (int)(Math.random() * 5 + 1) : 1 <= (int)(x * 5  + 1) < 6 // 1, 2, 3, 4, 5
		...
		System.out.printf("오늘이 점심 메뉴는 [%s] 입니다." , menu);
	...
	public void ex3() {
		...
		// 문자열로 메뉴를 입력받아 해당 메뉴에 가격을 출력
		// [실행화면]
		// 메뉴를 선택해주세요 (김밥, 라면, 떢복이) : 김밥
		// 김밥의 가격은 3000원 입니다.
		
		// 메뉴를 선택해주세요 (김밥, 라면, 떢복이) : 샌드위치
		// 존재하지 않는 메뉴입니다.
		
		Scanner sc = new Scanner(System.in);
		
		System.out.print("메뉴를 선택해주세요 (김밥, 라면, 떢볶이) : ");
		String menu = sc.next();
		...
		default : price = -1; // (임의로 정한)잘못 입력되었다는 신호 (중요!!!)
		}
	...
	public void ex4() {
		...
		int result = 0; // 계산 결과 저장용 변수 선언 및 초기화
						// -> 초기화를 했기때문에 switch문에서 값이 대입되지 않아도
