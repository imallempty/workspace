...
inputId.addEventListener("input", e => {
  ...
  if (val.length === 0) { // 입력된 값이 없을 경우
    ...
    // 유효성 검사 여부 저장 객체에서 "input-id"의 값을 false 변경(유효하지 않다를 뜻함)
...
inputPw.addEventListener("input", e => {
  ...
  if (regEx.test(val)) { // 유효한 경우
    span.innerText = "유요한 비밀번호 형식 입니다.";
    span.classList.add("check");
    span.classList.remove("error");
...
inputPwCheck.addEventListener("input", e => {
  ...
  // -> #input-pw의 다음, 다음 형제 요소(span)에 check 클래스가 있는 경우
...
function checkPwFn() {
  ...
  if (pwVal.length == 0 || checkVal == 0) { // 비밀번호 미입력 시
    span.innerText = "비밀번호를 먼저 입력해주세요";
    span.classList.remove("check");
    span.classList.remove("error");
    checkList["input-pw-check"] = false;
...
inputName.addEventListener("input", e => {
  ...
  if (val.length === 0) { // 입력된 값이 없을 경우
    span.innerText = "한글 2 ~ 15글자(단자음, 단모음 제외)";
    span.classList.remove("check");
    span.classList.remove("error");
