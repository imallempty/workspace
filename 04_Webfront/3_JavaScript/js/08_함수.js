...
// #increment 버튼 클릭 시 countIncrement 함수 코드 수행
...
/***** 변수 = 함수명 형식으로 함수를 선언한 경우 ******
   - 함수를 선언한걸로 인식하는 것이 아닌 변수를 선언한걸로 인식되어
     해석 순서 1번(함수 위치 인식)으로 인식되지 않아 함수 호출 코드가
     선언 코드보다 앞에 작성되면 오류 발생(호출 불가)
*/
...
arrowList[2].addEventListener("click", () => {
  ...
  // 다른 함수 호출
  // 매개 변수로 a,b 두 값을 전달 받아 곱한 값을 반환하는 익명 함수를
  // 전달
...
// return 하는 값이 object인 경우 {return} 생략 불가능
