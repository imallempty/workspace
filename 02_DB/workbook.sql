...
FROM TB_DEPARTMENT
GROUP BY DEPARTMENT_NO, CAPACITY,DEPARTMENT_NAME ;
...
WHERE EXTRACT (YEAR FROM ENTRANCE_DATE) = 2002 
	AND ABSENCE_YN = 'N' AND STUDENT_ADDRESS LIKE '%전주시%';
...
SELECT STUDENT_NAME "이름", COUNT(*) "동명이인 수"
FROM TB_STUDENT 
...
SELECT NVL(SUBSTR(TERM_NO,1,4), ' ') 년도 , 
	NVL(SUBSTR(TERM_NO,5,2), ' ') 학기,ROUND(AVG(POINT),1)
FROM TB_GRADE 
WHERE STUDENT_NO = 'A112113'
