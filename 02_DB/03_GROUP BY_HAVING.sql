...
SELECT SUN(SALARY) FROM EMPLOYEE; -- 1행
...
GROUP BY DEPT_CODE   -- DEPT_CODE가 같은 행끼리 그룹으로 묶어라
ORDER BY DEPT_CODE;  -- 조회 결과(RESULT SET)DMF DEPT_CODE 오름차순으로 정렬
...
FROM EMPLOYEE 
WHERE DEPT_CODE IN ('D5', 'D6') -- 부서코드가 'D5', 'D6'인 행남 남음
GROUP BY DEPT_CODE; -- 'D5', 'D6' 그룹으로 나뉨

-- EMPLOYEE 테이블에서 직급 별 2000년도 이후 입사자들의 급여 합을 조회
SELECT JOB_CODE, SUM(SALARY) 
FROM EMPLOYEE 
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000 /* HIRE_DATE >= '2000-01-01'도 가능 */
GROUP BY JOB_CODE;
...
SELECT DEPT_CODE 부서코드, SAL_LEVEL 급여등급, COUNT(*) 직원수
FROM EMPLOYEE
