...
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- EMPLOYEE DEPT_CODE와 DEPARTMEN의 DEPT_ID는 
-- 의미가 같은 컬럼(값도 같음)이기 때문에 두 컬럼을 이용해서
-- EMPLOYEE, DEPARTMENT 테이블을 하나로 합침
-- 하동운, 이오리는 DEPT_CODE가 NULL이기 때문에
-- DEPT_ID 중 일치하는 값이 없어 합친(JOIN)결과에 포함 X
...
/*1-1*/FROM EMPLOYEE 
/*1-2*/JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
...
-- 사번, 이름, 사수번호, 사수이름 조회
-- EMP_ID, EMP_NAME, MENAGER_ID, EMP_NAME
