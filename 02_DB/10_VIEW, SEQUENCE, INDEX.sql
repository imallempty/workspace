...
AS SELECT EMP_ID 사번, EMP_NAME 이름, 
	NVL(DEPT_TITLE, '없음') 부서명, JOB_NAME 직급명, NVL(LOCAL_NAME, '없음') 지역명
	FROM EMPLOYEE 
	NATURAL JOIN JOB 
	LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
--  원본 테이블에 필요한 컬럼, 행을 모아서 보여주는 가상의 테이블
...
WHERE TEST_ID = 'TEST500000'; -- 20 ms

-- 인덱스 사용 O
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 3/2/1 ms
