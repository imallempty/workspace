...
-- 테이블에서 조건을 충족하는 컬럼 값을 행을 조회할 때 사용
-- WHRER절에서는 조건식(true/false)만 작성
...
-- 사번, 이름 ,급여, 부서코드 조회
/* 3 */ SELECT EMP_NAME, EMP_ID, SALARY, DEPT_CODE 
/* 1 */ FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 급여가 300만원 미만 500만 초과인 사원의
-- 사번, 이름, 입사일, 급여 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY  
FROM EMPLOYEE
WHERE SALARY < 3000000 OR SALARY > 5000000

-- 컬럼명 NOT BETWEEN a AND b : a 이 b 이하이면 FALSE 
--> a 미만 b 초과면 TRUE 
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY  
FROM EMPLOYEE
...
--> 11행이 조회되어야 되는데 9행만 조회되었다!
-- 왜? DEPT_CODERK NULL 행이 2행이 있어서 그렇다!
...
-- EMPLOYEE 테이블에서 이름에 '하'로 포함되는 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME 
FROM EMPLOYEE 
WHERE EMP_NAME LIKE '%하%';

-- EMPLOYEE 테이블에서 이름에 '전'시작 '돈'으로 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME 
FROM EMPLOYEE 
...
--> 문제점 : 와일드 카드 표기법 '_'와
-- 		   기준으로 삼을 문자열 '_' 둘 다 와일드 카드로 해석되어
-- 		   조건식의 의마가 의도와 다르게 작성됨

--> 해결방법 : LIKE의 ESCPAE 옵션을 사용
--> ESCPAE 옵션 : 와일드 카드의 의미를 벗어나 단순 문자열로 인식
--> 	적용범위 : 특수문자 뒤 한 글자
SELECT EMP_ID, EMP_NAME, EMAIL 
FROM EMPLOYEE 
...
-- 날짜 형식의 문자열이기 때문에
-- HIRE_DATE (DATA 타입) 비교시 자동으로 형변환(Parsing)이 진행
...
-- SELECT절에서 해석된 별칭을 다음 순서인 ORDER BY 절에서 사용 가능

/* 정렬 시 컬럼값이 NULL인 행의 정렬 방법 지정*/
-- 모든 사원의 이름, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE 
--ORDER BY PHONE /*NULLS LAST*/; -- 오름차순 -> NULL을 마지막에 배치 (기본값)
--ORDER BY PHONE NULLS FIRST; -- 오름차순 -> NULL이 처음에 배치

-- ORDER BY PHONE DESC NULLS FIRST; -- 내림차순 -> NULL이 처음에 배치(기본값)
ORDER BY PHONE DESC NULLS LAST; -- 내림차순 -> NULL을 마지막에 배치

/* 정렬 중첩 */
-- 큰 분류를 먼저 정렬하고 
-- 작은 분류를 큰 분류가 흐트러지지 않은 선에서 정렬
