...
-- 단일행 함수 : N개의 행의 컴럼 값을 전달하여 N개의 결과가 반환
 
-- 그룹 함수 : N개의 행의 컴럼 값을 전달하여 1개의 결과가 반환
...
SELECT -123.5, 
	TRUNC(-123.5), -- -123 (0.5 버림)
	FLOOR(-123.5), -- -124 (0.5 내림)
...
-- ADD_MONTH(날짜, 숫자) : 날짜를 숫자만큼의 개 월수를 더하여 반환
SELECT SYSDATE, ADD_MONTHS(SYSDATE,1 ) "한달 후", 
		ADD_MONTHS(SYSDATE, -1 ) "한달 전"
FROM DUAL;

-- LAST_DAY(날짜): 해당 월의 마지막 일을 반환
...
-- 50 미만 : Y,R 둘 다 누락된 연도 앞부분에 현재 세기(2000년대)
-- 50 이상 : Y ->  현재 세기(2000년대) 추가
--			R -> 이전 세기(1900년대) 추가

-- 50 미만
SELECT TO_DATE('491125', 'YY-MM-DD') "YY",
       TO_DATE('491125', 'RR-MM-DD') "RR"
FROM DUAL;

-- 50 이상
SELECT TO_DATE('501125', 'YY-MM-DD') "YY",
       TO_DATE('1125', 'RR-MM-DD') "RR"
FROM DUAL;


-----------------------------------------------------

-- TO_NUMBER(문자열[,패턴]) : 문자열 -> 변환
...
SELECT EMP_ID, EMP_NAME, NVL2(PHONE, '010********', '없음')
FROM EMPLOYEE;
...
-- COUNT(DISTINCT 컬럼명) : 
-- 지정된 컬럼에서 중복값을 제외한 행의 개수를 반환
	-- EX) A A B C D D D D E : 5개 (중복은 한번만 카운트)
...
-- EMPLOYEE 테이블에 존재하는 여자사원의 수
SELECT COUNT(*) 
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8,1) = '2'; 

-- EMPLOYEE 테이블에 존재하는 남자사원의 수
SELECT COUNT(8) 
FROM EMPLOYEE
