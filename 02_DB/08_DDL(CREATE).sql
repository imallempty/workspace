...
INSERT INTO MEMBER
VALUES ('MEM01', '123ABC', '홍길동', '000930-1234567', DEFAULT);
...
--> SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(A230724_JDB.SYS_C0013076)에 위배됩니다
...
/* JOIN : 두 테이블에서 같은 값을 가지고 잇는 컬럼을 기준으로
 * (FK 제약조건이 설정되어있지 않아도 사용 가능
 * 
 * FK 제약 조건이 설정된 부모 - 자식 테이블
 * -> 무조건 JOIN 가능 
 * */ 
-- USER_UDED_FK, USER_GRADE 테이블 JOIN 
...
CREATE TABLE USER_USED_FK2(
  ...
  GRADE_CODE NUMBER,
  -- 테이블 레벨 FK + 삭제 옵션 설정
  CONSTRAINT GRADE_CODE_FK2 FOREIGN(GRADE_CODE) REFERENCES USER_GRADE2(GRADE_CODE) ON DELETE SET NULL
...
-- 자식 테이블 조회
SELECT * FROM USER_USED_FK2;
--> 홍길도, 이순시의 GRADE_CODE가 10 -> NULL 변화 확인
...
CREATE TABLE USER_USED_FK3(
  ...
  EMAIL VARCHAR2(50),
  -- 컬럼 레벨
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK3 REFERENCES USER_GRADE3 -- 참조할 컬럼을 작성하지 않으면 PK 자동 참조
  ON DELETE CASCADE
);
...
--> ON DELETE CASECADE 옵션이 설정되어 있어 오류없이 삭제 됨.
...
WHERE USER_NO = 1;
-- ORA-02290: 체크 제약조건(A230724_JDB.GENDER_CHECK)이 위배되었습니다
