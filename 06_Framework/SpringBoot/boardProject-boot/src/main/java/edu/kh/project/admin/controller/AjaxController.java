...
public class AjaxController {
	...
	public String selectMemberNo(int memberNo) {
		...
		/* !문제발생! */
		// - email을 반환 하려는데 Controller에서 반환된 값은 ViewResolver에 의해
		// 	 자동으로 /templets/반환값.html로 forward를 수행하는 문제 발생!
		
		// [해결방법]
		// Controller의 메서드 반환값이 forward를 위한 경로가 아닌 
		// 비동기 요청한 곳으로 돌아가야되는 값 자체임을 명시하는 
		// 어노테이션 @ResponseBody를 매서드 위에 추가
	}
	...
	@ResponseBody // forward 아니고 값 자체 반환 할꺼야
	...
	public Member selectMember(int no) {
		...
		// Java 객체는 JS에서 사용 불가능 -> 모양부터 다르기때문
		...
		// 2) JSON(문자열) 형태의 데이터를 JS로 전달(응답) + JSON이니깐 JS객체로 변환해서 사용하는 구문 첨부
	...
	public List<Member> selectAll() {
		...
		//   ( + produces 속성을 이용해 응답받는 JS에서 자동으로 JS 객체로 변환하도록 함) 
		// -> JS 객체 
	...
	// @RequestBody (요청 body)
	// - 요청 body에 담기 내용을 얻어와 오른쪽 매개변수에 대입
	// - HttpMessageConverter가 이 과정에서 데이터 타입을 Java에 맞게 변환
	//   number -> int/double
	//   string -> String
	//   JSON -> DTO, List, Map
