...
public class MemberController {
	...
	public String login(Member inputMember, HttpServletResponse resp,
		...
		if(loginMember != null) {
			...
			if(saveId != null) {
				...
				// 쿠키의 수명(유지기간) 지정 (초단위)
				cookie.setMaxAge(1*60*60*24*30); // 30일
			}
			...
			// 응답 객체(HttpServletResponse)를 이용해 서버 응답시 클라이언트에게 생성한 쿠키 전달
			// -> 전달 받은 클라이언트(브라우저)가 Cookie 객체를 브라우저 자체 쿠키 파일에 저장
	...
	public String quicklLogin(@PathVariable("memberEmail") String memberEmail,
			Model model, RedirectAttributes ra)  {
		
		Member loginMember = service.quicklLogin(memberEmail);
	 ...
	 * - String에서 비동기 통신 시 전달되는 데이터 자료형/ 응답하는 데이터의 자료형
	 ...
	 * 
	 * (참고)
	 * HttpMessageConverter가 독작하기 위해서는 Jackson-data-bind 라이브러리가 필요한데
