...
// - 파일 선택 화면에서 취소를 누르면 기존의 세팅된 value/files 가 없어지게 된다.
//  --> 1차 이미지 변경 후 2차 변경 과정에서 취소를 누르면 1차 value/files 가 없어지게 되어 제출 시 파일이 서버로 전달 x
...
if (imageInput != null) { // #imageInput 존재할 때
    ...
    const changeImgFn = e => {
        ...
        /* 이게 중요 */
        console.log(e.target.files); // 업로드된 파일의 정보가 담긴 배열 반환, 실제 파일
        ...
        if (uploadFile.sizw > maxSize) {
            alert("1MB 이하의 이미지만 업로드 가능합니다.");

            if (statusCheck == -1) { // 이미지 변경이 없었을 때

                // 최대 크기를 초과해도 input에 value가 남기 때문에 이를 제거하는 코드가 필요하다!
                imageInput.value = ''; // value 삭제, 동시에 files도 삭제
        ...
        reader.onload = e => {
            ...
            // img태그의 src 속성의 속성값으로 읽은 파일의 URL 대임
