...
boardLike.addEventListener('click', e => {
  ...
  // *** 로그인 여부 확인 방법 ***
  // session에 loginMember가 있으면 로그인 상태, 없으면 로그아웃
  // --> session을 이용해야하는데 JS만으로는 server session 사용 불가
  //     단, Thymeleaf를 작성할 수 있는 html 파일에서는 js와 혼용 가능

  // * html 파일에 boardDetail.js를 추가하기 전에 script 태그를 이용해서
  //   전역 변수로 로그인 여부를 저장 *
...
if(goToListBtn != null){ // 화면에 목록으로 버튼이 있을 때만 수행
  ...
  const gotoListFn = () => {
    ...
    // URL객체.searchParams : 주소에 담긴 쿼리스트링을 map형식으로 반환
    ...
    // URLSearchParams 객체 : 주소에서 쿼리스트링만 다루는 객체, 쿼리스트링 추가, 삭제, 제거, 생성 가능

    // 1) 쿼리 스트링 조합하기
    const tempParmas = new URLSearchParams();

    for(let key in obj){ // 객체 전용 향상된 for문
      if(obj[key] != null) tempParmas.append(key, obj[key]);
    }
  }
  
  // 2) 목록으로 돌아가기
  location.href = `/board/${boardCode}?${tempParmas.toString()}`;
