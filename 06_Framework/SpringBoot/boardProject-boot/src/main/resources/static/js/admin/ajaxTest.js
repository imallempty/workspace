...
selectMemberNo.addEventListener("click", () => {
  ...
  fetch("/ajax/selectMemberNo?memberNo=" + inputMemberNo.value)
    ...
    .then((email) => { // email : 첫번재 then()에서 파싱된 데이터
...
btn5.addEventListener("click", () => {
  ...
  fetch("/ajax/selectAll")
    .then(response => {
      ...
      // JS 형태의 응답 데이터를(memberList) JS 객체 형태로 변환해서 반환
      // (첫번째 then에서 리턴 시 promise 객체에 데이터만 반환)
...
btn6.addEventListener("click", () => {
  ...
  // JS 객체에 key, value 추가(JS 객체는 key가 존재하지 않으면 자동으로 추가되는 특징이 있음)
  member.memberEmail = sampleInputList[0], value;
  member.memberNickname = sampleInputList[1], value;
  member.Tel = sampleInputList[2], value;
